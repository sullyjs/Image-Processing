...
namespace INFOIBV
{
    public partial class INFOIBV : Form
    {
        ...
        private void applyButton_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            else if (CompleteHoughTransform.Checked)
            {
                ...
                List<Tuple<Tuple<int, int>, Tuple<int,int>>> lineSegments = new List<Tuple<Tuple<int, int>, Tuple<int,int>>>();
        ...
        private List<Tuple<Tuple<int, int>, Tuple<int, int>>> houghLineDetection(
            byte[,] image, Tuple<int, int> rTheta, int minThres, int minLen, int maxGap, int rOffset)
        {
            ...
            foreach(Tuple<int, int> onPixel in rthetaPixels)// int i = 0; i < rthetaPixels.Count; i++)
            {
                ...
                bool newPixelConnected = true;
                while (newPixelConnected)
                {
                    List<Tuple<int,int>> newlyFoundPxls = new List<Tuple<int, int>>();
                ...
                if(lineDistance >= minLen)
                {
                    lineSegments.Add(new Tuple<Tuple<int,int>, Tuple<int, int>>(startingPxl, endingPxl));
                }
                */
                ...
                {
                    lineSegments.Add(new Tuple<Tuple<int,int>, Tuple<int, int>>(onPixel, lastPxl));
        ...
        private byte[,] visualizeHoughLineSegments(byte[,] image, List<Tuple<Tuple<int, int>, Tuple<int, int>>> lineSegments)
        {
            byte[,] outputImage = image;
            // loop over all line segments
            foreach(Tuple<Tuple<int, int>, Tuple<int, int>> lineSegment in lineSegments)
            {
